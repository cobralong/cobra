...
(function($){
  ...
  $.fn.tagsManager = function (options,tagToManipulate) {
    var tagManagerOptions = {
      unEditablePrefilled: null,
      editablePrefilled:null,
      ...
      deleteTagsOnBackspace: null, // deprecated
      tagsContainer: null,
      tagCloseIcon: 'x',
      tagClass: '',
      validator: null,
    ...
    if (tagManagerOptions.hiddenTagListName === null) {
      tagManagerOptions.hiddenTagListName = this.attr('id');
    }

    var obj = this;
    var objName = obj.attr('id').replace(/[^\w]/g, '_');
    ...
    var pushTag = function (tag, classes) {
      tag = trimTag(tag);
      ...
      if (alreadyInList) {
        ...
        var html = '<span class="tm-tag ' + classes + '" id="' + newTagId + '">';
    ...
    var prefill = function (pta, classes) {
    ...
    var applyDelimiter = function (e) {
      ...
      if (!(e.which==13 && taItem && taVisible)) {
        pushTag(obj.val(), tagManagerOptions.tagClass);
      }
      e.preventDefault();
    };
    ...
    return this.each(function () {
      ...
      if (typeof options == 'string') {
        switch (options) {
          ...
          case "popTag":
            ...
            break;
          case "pushTag":
            pushTag(tagToManipulate, "tm-tag-warning");
            break;
        }
        return;
      }
      ...
      obj.change(function (e) {
        ...
        if (taItem && taVisible) {
          ...
          pushTag(taItem.attr('data-value'), tagManagerOptions.tagClass);
      ...
      if (tagManagerOptions.unEditablePrefilled != null) {
        if (typeof (tagManagerOptions.unEditablePrefilled) == "object") {
          prefill(tagManagerOptions.unEditablePrefilled, "tm-tag-disabled");
        } else if (typeof (tagManagerOptions.unEditablePrefilled) == "string") {
        	$.ajax({
                cache: false,
                type: "POST",
                contentType: "application/json",
                dataType: "json",
                url: tagManagerOptions.unEditablePrefilled,
                data: "",
                success: function (data) {prefill(data,"tm-tag-disabled"); }
              });
        } else if (typeof (tagManagerOptions.unEditablePrefilled) == "function") {
          prefill(tagManagerOptions.unEditablePrefilled(),"tm-tag-disabled");
        }
      }
      
      if (tagManagerOptions.editablePrefilled != null) {
          if (typeof (tagManagerOptions.editablePrefilled) == "object") {
            prefill(tagManagerOptions.editablePrefilled, "tm-tag-success");
          } else if (typeof (tagManagerOptions.editablePrefilled) == "string") {
          	$.ajax({
                  cache: false,
                  type: "POST",
                  contentType: "application/json",
                  dataType: "json",
                  url: tagManagerOptions.editablePrefilled,
                  data: "",
                  success: function (data) {prefill(data, "tm-tag-success"); }
                });
          } else if (typeof (tagManagerOptions.editablePrefilled) == "function") {
            prefill(tagManagerOptions.editablePrefilled(), "tm-tag-success");
          }
        }
