...
<title>应用汇-AppStore管理后台-客户端审核开关IP配置列表</title> <#include
"lib/base_source.ftl">
	...
	<div class="container-fluid">
							...
							<form method="post">
								<div class="input-prepend">
									<span class="add-on">客户端</span> <select name="bundleId">
										<#list bundleIdMap?keys as key> <#if key == para.bundleId>
										<option value="${key}" selected>${bundleIdMap[key]}</option>
										<#else>
										<option value="${key}">${bundleIdMap[key]}</option> </#if>
										</#list>
									</select>
								</div>
						...
						<div id="div_add" class="well form-inline">
							<div class='main-action'>
								<form id="searchForm" name="searchForm"
									...
									action="${rc.contextPath}/auth/store/client/audit/switch/ip/conf/add"
									method="post">
									<div class="input-prepend">
										<span class="add-on">客户端</span> <select name="bundleId">
											<#list pureBundleIdMap?keys as key> <#if key ==
											para.bundleId>
											<option value="${key}" selected>${pureBundleIdMap[key]}</option>
											<#else>
									...
									<div class="input-prepend">
										<span class="add-on">展示商店模式的IP:</span> <input type="text"
							...
							<form id="itemForm" name="itemForm" action="" method="post">
								...
								<table
									...
									<tbody class="sortable">
										...
										<tr>
											<td>${value.id}</td>
											<td>${pureBundleIdMap[value.bundleId]}----${value.bundleId}</td>
											<td>${value.ip}</td>
											<td><#if value.status == 0> <font
												id="status_font_${value.id?c}" color="green">√</font>
												<#else> <font id="status_font_${value.id?c}" color="red">X</font>
												</#if>
											</td>
		...
		<script>
			...
			$("span.delete").bind("click", function () {
		        ...
		        var postUrl = "${rc.contextPath}/auth/store/client/audit/switch/ip/conf/modify.json";
