            ...
            <#include "lib/alert.ftl">
                <div class='main-content'>
                    ...
                    <div id="div_add" class="well form-inline">
                        <div class='main-action'>
                            <form id="addForm" name="addForm" action="${rc.contextPath}/funny/client/type/add" method="post">
                            	<input type="hidden" id="pager_page" name="pager.page" value="${para.pager.page}">
		                        <input type="hidden" id="pager_size" name="pager.size" value="${para.pager.size}">
		                        <input type="hidden" id="pager_total" name="pager.total" value="${para.pager.total}">
                                <div class="input-prepend">
                                    <span class="add-on">类型名称</span>
                                    <input class="span2" name="name" type="text" placeholder="分类名称" value="">
                            	</div>
                            	<div class="input-prepend">
										<span class="add-on">应用推荐列表</span>
										<select name="appPromoteStatus">
											<option value ="0">存在</option>
											<option value ="-1">不存在</option>
										</select>
								</div>
								<div class="input-prepend">
										<span class="add-on">应用不推荐列表</span>
										<select name="appNotPromoteStatus">
											<option value ="0">存在</option>
											<option value ="-1">不存在</option>
										</select>
								</div>
                                ...
                                <#if values??>
                                    ...
                                    <tr id="tr_${value.id}">
                                    	<td>
                                         ...
                                         <td class="edit_td" default_id="${value.id?c}" for="name_${value.id?c}" name="name">
	                                        	<label for="name" class="span3">
	                                        	<span id="name_${value.id?c}" class="text-info span3" default_id = "${value.id?c}" pre_value="${value.name!}">
	                                        		${value.name!}
	                                        	</span>
	                                        	</label>
	                                    </td>
	                                    <td>
                                    	<#if value.appPromoteStatus == 0>
                                        	<button id="btn_appProList_${value.id?c}" class="btn btn-success delete" default_id="${value.id?c}" set_value="1" cur_value="0" flag="2">
                                        		存在
                                			</button>
                            			<#else>                                			
                                    		<button id="btn_appProList_${value.id?c}" class="btn btn-danger delete" default_id="${value.id?c}" set_value="0" cur_value="1" flag="2">
                                    			不存在                             	
                            				</button>
                                		</#if>
                                    	</td>
                                    	<td>
                                    	<#if value.appNotPromoteStatus == 0>
                                        	<button id="btn_appNotProList_${value.id?c}" class="btn btn-success delete" default_id="${value.id?c}" set_value="1" cur_value="0" flag="3">
                                        		存在
                                			</button>
                            			<#else>                                			
                                    		<button id="btn_appNotProList_${value.id?c}" class="btn btn-danger delete" default_id="${value.id?c}" set_value="0" cur_value="1" flag="3">
                                    			不存在                             	
                            				</button>
                                		</#if>
                                    	</td>
...
<script>
    ...
    $("button.delete").bind("click", function () {
        ...
        var name= $("#name_"+id).attr("pre_value");
        var trCtl = $("#tr_" + id);
        var hidden = $("#hidden_" + id);
        var flag = $(this).attr("flag");
        var postUrl = "${rc.contextPath}/funny/client/type/modify";
        ...
        bootbox.confirm(actionMsg, function (result) {
            ...
            var param ={};
           	if(flag==1){
           		param = {'id': id, "status": setStatus};
           	}else if(flag==2){
           		param = {'id': id, "appPromoteStatus": setStatus};
           	}else if(flag==3){
           		param = {'id': id, "appNotPromoteStatus": setStatus};
           	}else{
           		return;
           	}
