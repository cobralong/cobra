...
<script>
	$(function(){
		$("#ctypeId").change(function(){
			...
			if(p1=='应用集'){
				$("#applicationSetsTypeId").val(9999);
				$("#applicationSetsDiv").show();
			}else{
				$("#applicationSetsTypeId").val(9999);
				$("#applicationSetsDiv").hide();
			}
		});
	});
</script>
                        ...
                        <div class='main-action'>
                            <form id="searchForm" name="searchForm" action="" method="post">
                                ...
                                <div class="input-prepend">
                                    ...
                                    <input class="span2" name="title" style="width:485px;" type="text" placeholder="内容标题" value="${para.title}">
                                </div>
                                <div class="input-prepend">
                                    <span class="add-on">文章状态</span>
                                    <@spring.formSingleSelect "para.status", editStatusAll, " " />
                                </div>
                                <div class="input-prepend">
                                    <span class="add-on">分类</span>
                                    <@spring.formSingleSelect "para.ctypeId", ctypesall, " " />
                                </div></br>
                                <div class="input-prepend" id="applicationSetsDiv" style="display:<#if para.ctypeId==4>block<#else>none</#if>">
                                    <span class="add-on">应用集分类</span>
                                    <@spring.formSingleSelect "para.applicationSetsTypeId", applicationSetsall, " " />
                                </div>
                                <div class="input-prepend">
                                 ...
                                 <div class="input-prepend">
                                    ...
                                    <@spring.formSingleSelect "para.contentSrc", srcTypesall, " " />
                                </div>
                                <div class="input-prepend">
                            ...
                            <table class="table table-striped table-bordered table-condensed" id="itemTable">
                                ...
                                <#if values??>
                                    ...
                                    <tr id="tr_${value.id}">
                                        ...
                                        <td>
                                            <#if value.status == 0>
                                                <font id="font_status_${value.id}" color="red">${editStatusAll[value.status?c]}</font>
                                            <#elseif value.status == 1>
                                            	<font id="font_status_${value.id}" color="green">${editStatusAll[value.status?c]}</font>
                                            <#else>
                                            	<font id="font_status_${value.id}" color="red">X</font>
                                            </#if>
                                        </td>
                                        <td>
                                            <a href="${rc.contextPath}/auth/store/funny/column/content/detail?id=${value.id!}&type=1" type="button"
